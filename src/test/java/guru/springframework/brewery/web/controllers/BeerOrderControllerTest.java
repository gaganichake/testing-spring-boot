...
class BeerOrderControllerTest {
    ...
    void getOrder() throws Exception {
        ...
        given(beerOrderService.getOrderById(customerId, orderId)).willReturn(beerOrder);

        //when - then
        mockMvc.perform(get("/api/v1/customers/" + customerId.toString() + "/orders/" + orderId.toString()))
                .andExpect(status().isOk()) //then
                .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8)); //then
    }
}